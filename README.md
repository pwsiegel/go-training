...
The **"explore"** tsumego hints also run in the browser, so nothing analysis-related needs a backend. The private backend (`/api/katago`, started with `make api`, which also serves the Fox sync) is optional: when it's running, Review, Play, and Explore each offer it as an alternative full-strength **native KataGo (Metal)** engine in their model picker. The dev server proxies `/api/katago` and `/api/fox` to it.
